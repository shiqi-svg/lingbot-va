...
class FlexAttnFunc(nn.Module):
    ...
    def forward(
        ...
        # flex-attention 接口期望 [B, H, S, D]，这里做形状与精度整理。
    ...
    def init_mask(
        ...
        # 训练时按 latent/action 序列构造 block-sparse 掩码，控制可见性与因果约束。
...
class WanAttention(torch.nn.Module):
    ...
    def init_kv_cache(self, cache_name, total_tolen, num_head, head_dim,
        ...
        if self.attn_caches is None:
            return
        # 仅 self-attn 使用 KV cache；cross-attn 不缓存。
    ...
    def forward(
        ...
        if kv_cache is not None and kv_cache['k'] is not None:
            # 推理缓存路径：写入当前 token 的 KV，并取历史有效 token 参与注意力。
        ...
        if update_cache == 0:
            if kv_cache is not None and kv_cache['k'] is not None:
                # update_cache=0 代表仅临时使用，不保留当前写入槽位。
...
class WanTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        # time embedding 生成 AdaLN 风格的 scale/shift/gate，分别调制 SA 与 FFN。
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def _input_embed(self, latents, input_type='latent'):
        # latent: patchify 后线性映射；action: 按 token 映射；text: 文本投影。
    ...
    def _time_embed(self, timesteps, H, W, dtype, action_mode=False):
        # 将每帧时间步扩展到空间 token 粒度，再生成 temb 与 6 路调制参数。
    ...
    def forward_train(self, input_dict):
        # 训练分支：一次前向同时处理 latent/action 的 noisy+cond 四路序列。
        ...
        hidden_states = torch.cat([latent_hidden_states, #去噪对象
                                   condition_latent_hidden_states, #条件，VAE 编码的干净 latent
                                   action_hidden_states, #去噪对象
    ...
    def forward(
        ...
        # 推理分支一次只处理 latent 或 action 其中一条流。
