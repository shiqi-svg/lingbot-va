...
class Trainer:
    def __init__(self, config):
        ...
        self.step = 0
        self.config = config
        self.device = torch.device(f"cuda:{config.local_rank}") #确定当前进程设备与训练精度
        ...
        # 训练从预训练 transformer 出发，随后做 AC + FSDP 包装。
        ...
        # 视频 latent 与 action 分别使用独立 flow-matching 调度器。
        # 调度器作用：1.给样本加噪 2.训练目标是什么 3.不同时间步loss权重多少。 4.根据时间步还原回样本。
    ...
    def _add_noise(self, latent, train_scheduler, action_mask=False, action_mode=False, noisy_cond_prob=0.):
        # latent/action 统一形状约定: [B, C, F, H, W]。
        ...
        # noisy_cond_prob > 0 时，对条件分支也做噪声扰动，增强鲁棒性。
    ...
    def _prepare_input_dict(self, batch_dict):
        ...
        # chunk/window 在训练中随机采样，模拟不同自回归上下文长度。
        input_dict = {
    ...
    def convert_input_format(self, input_dict):
        ...
        # DataLoader 默认在 CPU，这里统一搬到当前 rank 对应 GPU。
    ...
    def compute_loss(self,
        ...
        # 模型输出序列格式，需还原回 [B, C, F, H, W] 与 target 对齐。
    ...
    def _train_step(self, batch, batch_idx):
        ...
        # FSDP 梯度累积优化: 非同步步跳过 all-reduce，减少通信开销。
        if not should_sync:
    ...
    def save_checkpoint(self,):
        ...
        try:
            # 从 FSDP 聚合 full state dict，并下转 bf16 减小存储体积。
